...
class ExpiredMedicinesWizard(models.TransientModel):
    ...
    def action_transfer_selected(self):
        """
        Create a single internal stock.picking for all selected lots,
        auto-validate it, and return a success notification.
        """
        self.ensure_one()
        ...
        if not selected_lines:
            raise UserError(_('No lots selected for transfer. Please select at least one expired medicine lot.'))

        # Group by source location → find the correct Expired destination per warehouse
        ...
        for picking_vals, move_vals_list in picking_data:
            ...
            # Confirm → auto-assign → validate
            picking.action_confirm()
            picking.action_assign()

            # Force done quantities
            for move in picking.move_ids:
                move.quantity = move.product_uom_qty

            picking.with_context(skip_expired_reason_check=True).button_validate()
            created_pickings |= picking

        count = len(selected_lines)
        _logger.info('SC2-UC-05: Bulk transferred %d expired lots — picking IDs: %s',
                     count, created_pickings.ids)

        # Reload wizard lines after transfer
        self.line_ids.unlink()
        lines = self._build_lines()
        self.line_ids = [(0, 0, l) for l in lines]
    ...
    def _prepare_picking_data(self, lines):
        """
        Group selected lines by (source_location, warehouse).
        For each group find the Expired destination location.
        Returns list of (picking_vals, [move_vals]) tuples.
        """
        # Group: key = (location_id.id)
        groups = {}
        for line in lines:
        ...
        for location_id, group_lines in groups.items():
            ...
            # Tag as Expired Transfer for traceability (if tag exists)
            tag = self.env['stock.picking.type'].search([], limit=1)  # placeholder
            # Add tag via context if available
            picking_vals['immediate_transfer'] = True
            ...
            for line in group_lines:
                move_vals_list.append({
                    ...
                    'location_id': source_location.id,
                    'location_dest_id': dest_location.id,
                    'lot_ids': [(4, line.lot_id.id)] if line.lot_id else [],
                    'description_picking': _(
                        'Expired lot: %s — Expiry: %s'
                    ) % (line.lot_id.name if line.lot_id else '', line.expiry_display),
                })
